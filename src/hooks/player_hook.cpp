...
void PlayerHook::install()
{
    ...
    o_OnWanted = reinterpret_cast<t_OnWanted>(
        GameBase::address(0x161C90)
        );

    HookManager::instance().addHook(
        reinterpret_cast<void**>(&o_OnWanted),
        reinterpret_cast<void*>(hk_OnWanted)
    );

    LOG_INFO("[PlayerHook] Player OnWanted hooked!");

    o_OnCrimeCommited = reinterpret_cast<t_OnCrimeCommited>(
        GameBase::address(0x162120)
    );
